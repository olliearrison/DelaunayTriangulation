...
inline bool inCircle(int v, const Triangle &t, const std::vector<Point> &V)
{
  assert(v >= 0 && v < V.size());
...
int replaceBoundaryHash(int t0Ind, Face f, int tInd, int v,
                        Mesh &M, std::vector<Point> &V, std::vector<char> &inserted)
{
...
{
  if (!hasEdge(T, a, b))
    T.graph[a].push_back(b);
  if (!hasEdge(T, b, a))
    T.graph[b].push_back(a);
  // T.graph[a].insert(b);
  // T.graph[b].insert(a);
}
...
std::pair<int, int> findBaseLR(const TriangulationDC &L, const TriangulationDC &R, const std::vector<Point> &V)
{
  ...
  while (changed)
  { // keep adjusting l and r until neither needs to move
    changed = false;
...
int findLeftCandidate(TriangulationDC &T, int l, int r, const std::vector<Point> &V)
{
  ...
  {
    if (potCand == r)
      continue; // skip current base LR point
...
TriangulationDC divideAndConquer(const std::vector<Point> &V, int lo, int hi)
{
  ...
  if (n <= 3)
  { // i.e. n = 2 (make line/edge) or n = 3 (make triangle)
...
std::vector<std::vector<int>> findFaces(const TriangulationDC &T, const std::vector<Point> &V)
{
  ...
  for (int start = 0; start < n; start++)
  {
    ...
    {
      if (visited.count({start, next}))
        continue; // skip if already used edge
      ...
      while (true)
      {
        ...
        if (u == start && v == next)
        { // returned to starting edge, so face complete
          break;
        }
      }
...
std::vector<TriangleDC> getTriangles(const TriangulationDC &T, const std::vector<Point> &V)
{
  ...
  for (auto &f : faces)
  {
    ...
    if (f.size() != 3)
      continue; // if face not size 3, prob outer boundary face
    ...
    if (o <= 0)
      continue; // not counterclockwise interior face then
...
int main(int argc, char *argv[])
{
  ...
  else if (parallel_mode == 'I')
  {
    ...
    std::vector<char> inserted(n, false);
